...
Создали ConfigMap из литералов
```bash
...
Создали ConfigMap из файла

Сделали файлик nginx.conf с простой конфигурацией:
```
server {
    listen 80;
...
Создали под config-demo, в котором три способа одновременно:
...
Запустили под с Secret

Создали под secret-demo, который в переменные окружения подтянул логин и пароль.
```bash
...
Создали Deployment с Postgres, который:
